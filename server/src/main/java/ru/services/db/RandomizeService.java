...
@Slf4j
@Service
public class RandomizeService implements IRandomizeService {
    ...
    private List<String> getRandSchedule(School school, Group group, List<User> tea){
        ...
        final List<User> teachers = school.getTeachers();
        ...
        for(day = 0; day < 5; day++) {
            ...
            for(les = 0; les < 6; les++) {
                ...
                final Role roleTeacher = teaU.getRole(Roles.TEACHER);
                if(!ObjectUtils.isEmpty(teachers) && teachers.contains(teaU)){
                    teachers.remove(teaU);
                }
                if(!roleTeacher.getSubjects().contains(nameSubj)) {
                    roleTeacher.getSubjects().add(nameSubj);
                    roleRepository.saveAndFlush(roleTeacher);
                }
                lesson = lessonRepository.saveAndFlush(new Lesson(school, group, day, les, kab, nameSubj, teaU));
