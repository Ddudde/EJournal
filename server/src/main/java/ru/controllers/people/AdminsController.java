...
@RestController public class AdminsController {
    ...
    public ResponseEntity<Void> remPep(@RequestBody AdminsInnerDTO body, AuthToken auth) {
        ...
        sseService.sendEventFor(auth.getUserId(), "remPepC", outDTO, TypesConnect.ADMINS, "main", "main", "main", "main");
    ...
    @PreAuthorize("""
        ...
        and hasAuthority('ADMIN')""")
    @PatchMapping("/chPep")
    public ResponseEntity<Void> chPep(@RequestBody AdminsInnerDTO body, AuthToken auth) {
        final User user1 = dbService.userById(body.id);
        if (user1 == null) return ResponseEntity.notFound().build();

        final AdminsOutDTO outDTO = adminsService.changeFIO(user1, body.name);
        sseService.sendEventFor(auth.getUserId(), "chPepC", outDTO, TypesConnect.ADMINS, "main", "main", "main", "main");
        return ResponseEntity.ok().build();
    }
    ...
    @PreAuthorize("""
        ...
        and hasAuthority('ADMIN')""")
    @PostMapping("/addPep")
    public ResponseEntity<Void> addPep(@RequestBody AdminsInnerDTO body, AuthToken auth) {
        final Syst syst = dbService.getSyst();
        ...
        sseService.sendEventFor(auth.getUserId(), "addPepC", outDTO, TypesConnect.ADMINS, "main", "main", "main", "main");
    ...
    public ResponseEntity<Map<Long, UserServiceBodyUserDTO>> getAdmins(AuthToken auth) {
        final Syst syst = dbService.getSyst();
        ...
        sseService.changeSubscriber(auth.getUUID(), TypesConnect.ADMINS, "null", "main", role, "main");
