...
@RestController public class PeriodController {
    ...
    @PostMapping("/addPer")
    public ResponseEntity<Void> addPer(@RequestBody PeriodsInnerDTO body, AuthToken auth) {
        ...
        sseService.sendEventFor(auth.getUserId(), "addPerC", outDTO, TypesConnect.PERIODS, school.getId() +"", "main", "main", "main");
    ...
    @PreAuthorize("""
        ...
        and hasAuthority('HTEACHER')""")
    @GetMapping("/getInfo")
    public ResponseEntity<PeriodOutDTO> getInfo(AuthToken auth) {
        final User user = dbService.userById(auth.getUserId());
        final School school = user.getSelecRole().getYO();
        ...
        sseService.changeSubscriber(auth.getUUID(), TypesConnect.PERIODS, school.getId() +"", "main", "main", "main");
