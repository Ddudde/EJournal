...
import ru.data.DTO.service.data.initDB.InitDBServiceBodyGroupDTO;
import ru.data.DTO.service.data.initDB.InitDBServiceBodySchoolDTO;
 ...
 * toDo: Пересмотреть геттеры/сеттеры */
@Slf4j
@Getter @Setter
...
@Service public class InitDBService {
    ...
    private void checkDates() {
        try {
            ...
            for(User user : listUsers) {
                if(!ObjectUtils.isEmpty(user.getExpDate()) && now >= AppConfig.dataFormat.parse(user.getExpDate()).getTime()){
                ...
                if(settingUser != null && !ObjectUtils.isEmpty(settingUser.getExpDateEC()) && now >= AppConfig.dataFormat.parse(settingUser.getExpDateEC()).getTime()){
        ...
        if(random.nextBoolean()) {
            ...
            final User user = userRepository
                .saveAndFlush(new User(fio, Map.of(
                    selRole, role
                ), selRole, AppConfig.dataFormat.format(dateAfter), uuid));
            users.add(user.getId());
            return user;
        } else {
            final String login = UserService.getRandomUsername(fakerEn);
    ...
    private void getRandomMark(Group group, School school, String nameSubj, int dayOfWeek, User teaU) {
        final Period period = periodService.getActualPeriodBySchool(school);
...
//            settingUserRepository.deleteAll(setts);
    ...
    private Map<Long, InitDBServiceBodyDTO> getUsersForTestDTO(List<User> users, Map<Long, InitDBServiceBodyDTO> usersByIdDTO) {
        for (User user : users) {
            final InitDBServiceBodyDTO.InitDBServiceBodyDTOBuilder dtoBuilder = InitDBServiceBodyDTO.builder();
    ...
    private Map<Long, InitDBServiceBodyDTO> getUsersForTestDTO(List<User> users) {
     ...
     * @see TestController#getInfo(SubscriberDTO, CustomToken)  Пример использования */
    public InitDBServiceDTO getTestInfo() {
        final var dtoBuilder = InitDBServiceBodyDTO.builder();
        final Map<Long, InitDBServiceBodySchoolDTO> schoolsByIdDTO = new HashMap<>();
        ...
        if(ObjectUtils.isEmpty(schools)) {
            return new InitDBServiceDTO(dtoBuilder.build());
        }
    ...
    private void prepareSchoolToDTO(School school, Map<Long, InitDBServiceBodySchoolDTO> schoolsByIdDTO) {
        final var schoolDTOBuilder = InitDBServiceBodySchoolDTO.builder();
        final List<User> teachersUBySchool = lessonRepository.uniqTeachersUBySchool(school.getId());
        final Map<Long, InitDBServiceBodyGroupDTO> groupsByIdDTO = new HashMap<>();
        ...
        for (Group group : school.getGroups()) {
            final var groupDTOBuilder = InitDBServiceBodyGroupDTO.builder();
            final Map<Long, InitDBServiceBodyDTO> parentsById = new HashMap<>();
